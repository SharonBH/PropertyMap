    ...
    : IRequestHandler<CreateAgencyCommand, CreateAgencyResponse>
{
    public async Task<CreateAgencyResponse> Handle(CreateAgencyCommand request, CancellationToken cancellationToken)
    {
        ...
        var agency = Agency.Create(request.Name!, request.Email!, request.Telephone!, request.Address!, request.Description, request.LogoURL, request.PrimaryColor, request.AdditionalInfo);
        await repository.AddAsync(agency, cancellationToken);
        logger.LogInformation("Agency created {AgencyId}", agency.Id);
        return new CreateAgencyResponse(agency.Id, agency.Name, agency.Email, agency.Telephone, agency.Address, agency.Description, agency.LogoURL, agency.PrimaryColor, agency.AdditionalInfo);
    }
}
